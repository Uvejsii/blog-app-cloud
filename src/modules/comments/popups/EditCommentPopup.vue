...
import {editComment} from "@/modules/comments/sdk/api.js";
...
const { mutate, isPending } = useMutation({
  mutationFn: () => editComment(values),
  onSuccess: () => {
    queryClient.invalidateQueries({
      queryKey: props.data.commentKey,
    })
    emit('close', values)
    toast.add({
      severity: 'success',
      summary: 'Comment Edited Successfully',
      life: 5000
    })
  },
  onError: (error) => {
...
<template>
  ...
  >
    ...
    <template #footer>
      ...
      <Button @click="submitForm" :disabled="isPending">{{ isPending ? 'Editing...' : 'Edit' }}</Button>
    </template>
  </Dialog>
</template>
